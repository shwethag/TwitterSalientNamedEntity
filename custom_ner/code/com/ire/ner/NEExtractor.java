...
/**
 * _USR (user eg @shwetha) _HT (hash tag eg #felicity) _NN(P) (eg Boehner)
...
public class NEExtractor {
	...
	private String getNEs(String tweet) {
		...
		for (i = 0; i < words.length; i++) {
			...
			} else if (word.contains(_NN) && Character.isUpperCase(word.charAt(0))) {
				while (word.contains(_NN) && Character.isUpperCase(word.charAt(0))) 
				{
					nes += word.substring(0, word.indexOf(_)) + " ";
					i++;
					// More than one NEs could be separated by , or .
					if (word.contains(COMMA) || word.contains(FULL_STOP) || word.contains(QM) || word.contains(EXCL)) {
